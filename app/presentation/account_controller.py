...
from app.domain.schemas.account_schema import AccountCreateSchema, AccountSchema
...
def update_account(account: AccountCreateSchema, account_id: int = Depends(get_account_id_from_token), usecase: AccountUseCase = Depends(AccountUseCase)):
    return usecase.update_account(account_id, Account(**account.model_dump()))
